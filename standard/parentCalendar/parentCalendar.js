...
_getEventHash: function(getEventsJson) {
    ...
    this._updateFilterPanel(filter);
...
_getEvent: function(json, id) {
    ...
    var contentEvent = { o_field_settings: { yglui_str_wid_fs_record: appids[appidFound] },
        o_field_values: { yglui_str_wid_record: events[eventFound] },
...
createFilterPanel: function(element, appName, incapp) {
    ...
    var _panelStructure = [{
        eventTypes: {
            groupTitle: 'Event types',
            filterOptions: appids
        }
    },
        {
            eventTypes: {
                groupTitle: 'Event status',
                filterOptions: statuses
            }
}];
    var tableHeader = new Element('thead').insert('<tr></tr>');
    var tableBody = new Element('tbody').insert('<tr></tr>');
    _panelStructure.each(function(item) {
        var title = item.eventTypes.groupTitle;
        var filterOptions = item.eventTypes.filterOptions;
    ...
    var jsonButton = {
        elements: [],
        mainClass: 'timeFilter_buttonDiv'
    };
    var aux = {
    ...
    $(element).insert(filterButton.getButtons());
...
getFilterSelectedOptions: function() {
    ...
    result.set('string', returnSelected);
    var xmlParser = new XML.ObjTree();
    var json = xmlParser.parseXML("<o_li_incapp>" + returnSelected + "</o_li_incapp>");
    result.set('object', json.o_li_incapp);
    return result;
},
/**
