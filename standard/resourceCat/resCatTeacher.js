/*
/** 
* @fileOverview resCatTeacher.js 
* @description File containing class resCatTeacher. 
* Application for Resource Catalogue.
*/

/**
*@constructor
*@description Class resCatTeacher.
*/
var resCatTeacher = Class.create(GenericCatalog, {

    getCatSessionsService: 'GET_CATSESSION',
    getNavLinkService: 'GET_MAIN_LINK',
    CLASS_OBJTYPE: $H({
        P: 'learning_internalTeacherIcon',
    ...
    */
    initialize: function($super, options) {
        $super(options, {
            ...
            applicationId: 'resCatTeacher'  //applicationId: args.className
        });
    ...
    */
    setLegendDiv: function() {
        ...
        objectToArray(this.json.EWS.o_legend.item).each(function(element) {
            ...
            if (element['@otype'] == 'P')
                text = global.getLabel('P');
            aux.legend.push({ img: this.CLASS_OBJTYPE.get(element['@otype']), text: text });
        } .bind(this));
        this.virtualHtml.down('div#' + this.applicationId + '_level4').update(getLegend(aux, 2));
    ...
    actionClicked: function(parameters) {
        ...
        switch (name) {
            ...
            case "LSODELETEH":
                ...
                this.genericDelete('H', nodeId, name, nextApp, global.getLabel('delete_resourceInfo'), okCode);
                balloon.hide();
                break;
            case "LSODELETERELATP":

                // this.deleteRelatP('P',nodeId,name,nextApp,global.getLabel('delete_resourceInfo'),okCode);
                balloon.hide();
                global.open($H({
                    app: {
                        appId: nextApp,
            ...
            case "LSODISPLAYH": // show courses
                ...
                break;
            case "LSOMAINTAINH": // show courses
                ...
                break;
            case "LSOCREATERELATP": //assign internal person
                balloon.hide();
                global.open($H({
                    app: {
                        appId: nextApp,
