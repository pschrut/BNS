     ...
     if (this.widgetsStructure.get('structure')['halfSize'].length > 0) {
         ...
         for (var i = 0; i < 2; i++) {
             this.virtualHtml.insert(
               "<div id='getWidgets_widget_" + i + "' class='getWidgets_column'></div>"
            );
         }
 ...
 */
 createWidgets: function() {
     ...
     if (this.widgetsStructure.get('structure')['halfSize'].length > 0) {
         ...
         this.portalHalf = new Widgets.Portal('#' + this.virtualHtml.identify() + ' div', 'getWidgetsHalf_' + this.tabId + '_portal', {});
         for (var i = 0; i < widgetsHalfSize.length; i++) {
             ...
             if (!Object.isEmpty(widgetsHalfSize[i]['column1'])) {
                 ...
                 var auxWidget = new Widgets.Widget('widget_' + widgetsHalfSize[i]['column1']['@appid'], { optionsButton: false, closeButton: false }).setTitle(label);
             ...
             if (!Object.isEmpty(widgetsHalfSize[i]['column2'])) {
                 ...
                 var auxWidget = new Widgets.Widget('widget_' + widgetsHalfSize[i]['column2']['@appid'], { optionsButton: false, closeButton: false }).setTitle(label);
     ...
     if (this.widgetsStructure.get('structure')['fullSize'].length > 0) {
         ...
         this.portalFull = new Widgets.Portal('#' + this.virtualHtmlFullSize.identify() + ' div', 'getWidgetsFull_' + this.tabId + '_portal', {});
         for (var i = 0; i < widgetsFullSize.length; i++) {
             var label = this.labels.get(widgetsFullSize[i]['@appid']);
             var auxWidgetFull = new Widgets.Widget('widget_' + widgetsFullSize[i]['@appid'], { optionsButton: false, closeButton: false }).setTitle(label);
