...
_setInitialHTML: function() {
    ...
    var html = "<div id='OMdisplay_content'>" +
                       ...
                       "</div>" +
                       "<div id='radioButtons'>" +
                           "<div id='OMdisplay_form_orgUnit' class='fieldDispFloatLeft'>" +
                                "<input id='OMdisplay_orgUnit' type='radio' name='gcRadioGroup' value='orgUnit' checked>" + global.getLabel('ORGEH') + "</input>" +
                           "</div>" +
                           "<div id='OMdisplay_form_position' class='fieldDispFloatLeft'>" +
                                "<input id='OMdisplay_position' type='radio' name='gcRadioGroup' value='position'>" + global.getLabel('PLANS') + "</input>" +
                           "</div>" +
                       "</div>" +
    ...
    this.htmlContent.down('div#button_OMdisplay_search').observe('click', function() {
...
_getInitialChart: function() {
    var xml = "<EWS><SERVICE>" + this.getOMService + "</SERVICE>" +
            ...
            "<o_date>" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "</o_date>" +
            "<o_depth>2</o_depth>" +
            "<o_mode>D</o_mode>" +
            "</PARAM></EWS>";
...
_drawNode: function(node, type, targetDiv) {
    ...
    for (var i = 0; i < managers.length; i++) {
        ...
        html += "'";
        if (!Object.isEmpty(managername)) {
            //html += " onClick='javascript:document.fire(\"EWS:openApplication\", $H({mode:\"popUp\", app:\"OM_MaintObjView\", node: \"" + managers[i].get("employeeid") + "\", objectType:\"P\", begdate: \"" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "\", position: \"" + managers[i].get("positionid") + "\"}))'";
            var views = 'DisplayPerson';
            var tartb = 'POPUP';
            var requestAppId = 'OM_PDIS';
            //for old OM - showing details without FP
            //html += " onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, node:\"" + managers[i].get("employeeid") + "\", objectType:\"P\", begdate:\"" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "\", position:\"" + managers[i].get("positionid") + "\" }))'";
            //for old OM - using get_content2 service and FP
            html += " onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, parentType:\"P\", begda:\"" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "\", displayMode:\"display\", objectId:\"" + managers[i].get("employeeid") + "\" }))'";
        }
        ...
        if (!Object.isEmpty(managerpos)) {
            ...
            var views = 'DisplayPosition';
            var tartb = 'POPUP';
            var requestAppId = 'OM_SDIS';
            //for old OM - showing details without FP
            //html += " onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, node:\"" + managers[i].get("positionid") + "\", objectType:\"S\", begdate:\"" + managers[i].get("begda") + "\", enddate:\"" + managers[i].get("endda") + "\", root:\"" + node.get("orgunitname") + "\" }))'";
            //for old OM - using get_content2 service and FP
            html += " onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, parentType:\"S\", begda:\"" + managers[i].get("begda") + "\", displayMode:\"display\", objectId:\"" + managers[i].get("positionid") + "\" }))'";
        }
        html += ">" + managerpos + "</span><br />";
    }
    html += "</td></tr>";
    // Links and arrow
    var views = 'DisplayOrgUnit';
    var tartb = 'POPUP';
    var requestAppId = 'OM_ODIS';
    html += "<tr>" +
    ...
    //for old OM - showing details without FP
    //"<td class='OMdisplay_widget_initialInfo_link'><span class='application_action_link' onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, node:\"" + node.get("orgunitid") + "\", objectType:\"O\", begdate:\"" + node.get("begda") + "\", enddate:\"" + node.get("endda") + "\", root:\"" + node.get("orgunitname") + "\" }))'>" + global.getLabel('more') + "</span>&nbsp;&nbsp;";
    //for old OM - using get_content2 service and FP
    "<td class='OMdisplay_widget_initialInfo_link'><span class='application_action_link' onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, parentType:\"O\", begda:\"" + node.get("begda") + "\", displayMode:\"display\", objectId:\"" + node.get("orgunitid") + "\" }))'>" + global.getLabel('more') + "</span>&nbsp;&nbsp;";
...
_drawPositions: function(node, targetDiv) {
    ...
    for (var i = 0; i < positions.length; i++) {
        ...
        html += "'";
        if ((!Object.isEmpty(employeename)) && (employeeid != '00000000') && (status != 'E') && (status != 'O')) {
            //html += " onClick='javascript:document.fire(\"EWS:openApplication\", $H({mode:\"popUp\", app:\"OM_MaintObjView\", node: \"" + positions[i].get("employeeid") + "\", objectType:\"P\", begdate: \"" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "\", position: \"" + positions[i].get("positionid") + "\"}))'";
            var views = 'DisplayPerson';
            var tartb = 'POPUP';
            var requestAppId = 'OM_PDIS';
            //for old OM - showing details without FP
            //html += " onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, node:\"" + positions[i].get("employeeid") + "\", objectType:\"P\", begdate:\"" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "\", position:\"" + positions[i].get("positionid") + "\" }))'";
            //for old OM - using get_content2 service and FP
            html += " onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, parentType:\"P\", begda:\"" + this.formDatePicker.getActualDate().toString(this.dateFormat) + "\", displayMode:\"display\", objectId:\"" + positions[i].get("employeeid") + "\" }))'";
        }
        ...
        var requestAppId = 'OM_SDIS';
        //for old OM - showing details without FP
        //html += "'><span class='application_legend_text OMdisplay_widget_initialInfo_italic application_action_link OMdisplay_widget_no_underline' onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, node:\"" + positions[i].get("positionid") + "\", objectType:\"S\", begdate:\"" + positions[i].get("begda") + "\", enddate:\"" + positions[i].get("endda") + "\", root:\"" + node.get("orgunitname") + "\" }))'>";
        //for old OM - using get_content2 service and FP
        html += "'><span class='application_legend_text OMdisplay_widget_initialInfo_italic application_action_link OMdisplay_widget_no_underline' onClick='javascript:global.open($H({app: { appId: \"" + requestAppId + "\",tabId: \"" + tartb + "\",view: \"" + views + "\"}, parentType:\"S\", begda:\"" + positions[i].get("begda") + "\", displayMode:\"display\", objectId:\"" + positions[i].get("positionid") + "\" }))'>";
...
_callToGetOptionsSearch: function() {
    ...
    this.virtualHtml.down('[id = radioButtons]').childElements().each(function(radioButton) {
        if (radioButton.down().checked)
            objectTypeSearch = radioButton.down().value;
    } .bind(this));
    var orgUnitChecked = "";
    var posChecked = "";
    if (objectTypeSearch == 'orgUnit') {
        orgUnitChecked = 'Y';
        posChecked = 'N';
    } else {
        orgUnitChecked = 'N';
    ...
    var parsedDate = this.formDatePicker.getActualDate().toString(this.dateFormat);
    // Call to the service
    var xml = "<EWS>" +
