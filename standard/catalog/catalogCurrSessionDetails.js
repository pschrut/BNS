var CATL_CUR = Class.create(getContentDisplayer, {
    ...
    */
    run: function($super,args) {
        var buttonsHandlers = $H({
            APP_LSOSAVECUR: function() { this.saveRequest('APP_LSOSAVECUR') } .bind(this),
            SCR_LSOCUCONTENT: function() { this.fp.displaySecondaryScreens("2"); } .bind(this),
            cancel: function() {
    ...
    */    
    setTitle: function(){
        var label;
        if(this.mode == 'edit')
            label = global.getLabel('maintCU');
        else if (this.mode == 'create')
            label = global.getLabel('createCU');
        else if (this.mode == 'display')        
            label = global.getLabel('displayCU');        
    ...
    */
            ...
            if (fieldsRequest.length > 0) {
                var xml = '<EWS>' +
                            ...
                            '<PARAM>' +
                                ...
                                //if TM_L_CU, we insert the I_PARENT
                                    xml += '<I_PARENT>' + this.objectId + '</I_PARENT>';                                       
                                xml += '<RECORDS>' +
                                    fieldsRequest +
                                '</RECORDS>' +
    ...
    linkButtons: function() {
        if(!Object.isEmpty(this.virtualHtml.down("[id="+ this.appName+"_SCR_LSOCUCONTENT]"))){
            this.virtualHtml.down("[id="+ this.appName+"_SCR_LSOCUCONTENT]").stopObserving('click');	
            this.virtualHtml.down("[id="+ this.appName+"_SCR_LSOCUCONTENT]").observe('click',function(){this.displaySecondaryScreens(1);}.bind(this));	
        }
    }, 
    /*
    * @method close
