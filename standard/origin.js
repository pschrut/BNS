 ...
 */
var __logOnUrl = "/sap/bc/bsp/sap/yglui_bsp_login?sap-exiturl=../../../../../sap/bc/bsp/sap/yglui_bsp_ews/index.html&sap-client=300";
...
//var __hostName = "proxy.aspx?url=" + escape("http://eu2r3edn.euhreka.erp:8003/sap/bc/yglui_httpentry?sap-client=300");
var __hostName = "/sap/bc/yglui_httpentry";
/******************************/
/*   EDC 310 - STANDARD       */
/******************************/
...
function iFrameToSelect(element){
    ...
    if (isIE) {
        ...
        element.insert
        ({
            before: '<iframe id="' + id + '" src="blank.html" scroll="no" frameborder="0" ' +
...
function displayToLong(displayFormatNumber) {
	//See which is the separator for decimals
	var decimalSeparator = ",";
	var thousandSeparator = ".";
	if (!Object.isEmpty(global.numberFormat)) {
		var formatArray = global.numberFormat.toArray();
		var lastComma = -1;
		var lastDot = -1;
		for(var i=0; i<formatArray.size(); i++){
			if(formatArray[i]==","){
				lastComma = i;
			}else if(formatArray[i]=="."){
				lastDot = i;
			}
		}
		//If the dot should be the decimal separator
    ...
    //TODO: if the number has the separators the other way, change them?
	
	//Remove the thousand separator
    var stringNumber = displayFormatNumber.gsub(thousandSeparator, "");
	//Use the dot as decimal separator (as needed by SAP)
	stringNumber = stringNumber.gsub(decimalSeparator, ".");
	
	//Check if it is a valid number (we use dot as decimal separator since it's javascript standard
    var floatNumber = parseFloat(stringNumber);
    if (floatNumber == null || isNaN(floatNumber)) {
        return NaN;
    }
...
function getLegend(options, cols) {
    ...
    var showLabelDiv = new Element('span', ({
        'id': 'legend_module_showLabel',
        'class': 'application_action_link'
    })).update(showLabel);
    var hideLabelDiv = new Element('span', ({
        'id': 'legend_module_hideLabel',
        'class': 'application_action_link'
    })).update(hideLabel);
    ...
    for (var i = 0; i < legend.length; i++) {
        ...
        }).insert('<div class=' + legend[i]['img'] + ' legend_module_column_' + (i % cols) + '></div>');
...
function underlineSearch(str, search, emphasizeClass) {
    ...
    if(!Object.isEmpty(search[0])) {
        ...
        for (var i = 0; i < search.length; i++) {
            var index = str.toLowerCase().indexOf(search[i]);
...
_failureMethod: function(data) {
    ...
    if (data) {
        ...
        errorText = "Application is currently experiencing connection problems. Please try again later.";
    }
...
* @description this method show the default error, warning, and info text messages
*/
_errorMethod: function(args) {
    this._failureMethod(args);
}
});
...
initialize: function(object, options) {
    ...
    //if it's not in the cache make an AJAX request to get the data
        ...
        if (!Object.isEmpty(global) && !Object.isEmpty(global.client)) {
            //We add the lenguage as a parameter in the url
            if (url.include("%3F")) {
                //If the url already has parameters (has the ? character)
        ...
        var AJAXREQ = new Ajax.Request(url, {
            ...
            asynchronous: true,
            postBody: processedOptions.xmlIn,
            ...
            onFailure: function(req) {
                object._failureMethod(data, processedOptions.ajaxID);
            }
        });
    }
},
...
processXmlIn: function(options) {
    ...
    var xmlParser = new XML.ObjTree();
    var xmlJson = xmlParser.parseXML(options.xmlIn);
