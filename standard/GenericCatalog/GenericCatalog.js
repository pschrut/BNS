...
/**
*@type Hash
*@description It keeps the mapping between object types and CSS classes to apply them.
...
setLegendDiv: function() {
    ...
    this.json.EWS.o_legend.item.each(function(element) {
        aux.legend.push({ img: this.CLASS_OBJTYPE.get(element['@otype']), text: element['@wegnr'] });
    } .bind(this));
...
nodeSearch: function(args) {
    ...
    if (this.makeCall) {
        ...
        if (getArgs(args).idAutocompleter == this.applicationId + '_autocompleter') {
            ...
            autoCompText = (((autoCompText.split(global.idSeparatorLeft))[0].split('['))[0].split('<'))[0].strip();
            var xml = "<EWS>" +
