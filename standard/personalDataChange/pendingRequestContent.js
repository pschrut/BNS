...
var PendingRequestContent = Class.create(origin, {
    ...
    */
    fillPendingRequestWidget: function(appId) {
        ...
        var html = "<div id='contentPart'>" +
                        ...
                        "<div id='searchPart' class='pdcPendReq_searchPart'></div>" +
    ...
    */
    processPendingRequest: function(json) {
        if (!Object.isEmpty(json.EWS.o_records)) {
            ...
            this.labelsPendReq = objectToArray(json.EWS.labels.item);
            ...
            var id, appId, datum, action, status, datumMonth;
    ...
    */
    drawNavigationLink: function() {
        ...
        this.virtualHtml.down('span#link_' + this.actualLink).removeClassName('application_action_link');
        this.virtualHtml.down('span#link_' + this.actualLink).addClassName('application_text_bolder');
        //stop the event for the selected link
        this.virtualHtml.down('span#link_' + this.actualLink).stopObserving('click');
    },
    /**
    ...
    */
    updateInfo: function(link) {
        ...
        this.virtualHtml.down('span#link_' + this.oldLink).removeClassName('application_text_bolder');
        this.virtualHtml.down('span#link_' + this.oldLink).addClassName('application_action_link');
        this.virtualHtml.down('span#link_' + this.actualLink).removeClassName('application_action_link');
        this.virtualHtml.down('span#link_' + this.actualLink).addClassName('application_text_bolder');
        //update events of links
        this.virtualHtml.down('span#link_' + this.oldLink).observe('click', this.updateInfo.bind(this, this.oldLink));
        this.virtualHtml.down('span#link_' + this.actualLink).stopObserving('click');
    ...
    */
    drawSearchField: function() {
        ...
        var complexSearch = new Element('input', {
            'id': 'PendReq_searchBox',
            'type': 'text',
            'class': 'application_autocompleter_box menus_myTeamField pdcPendReq_searchElement',
            'value': global.labels.get('search')
        });
        this.virtualHtml.down('[id=searchPart]').insert(complexSearch);
        //events
        this.virtualHtml.down('[id=PendReq_searchBox]').observe('keyup', this.fieldKeyUp.bindAsEventListener(this));
        this.virtualHtml.down('[id=PendReq_searchBox]').observe('blur', function() {
            if (complexSearch.value == '') complexSearch.value = global.getLabel('search');
        } .bindAsEventListener(this));
        this.virtualHtml.down('[id=PendReq_searchBox]').observe('focus', this.fieldFocus.bindAsEventListener(this));
    },
    /**
    ...
    */
    startNewDataTimeout: function(instance) {
        ...
        instance.applyFilters(instance.textSearch);
    },
    /**
    ...
    */
    fieldKeyUp: function(event) {
        ...
        if (this.timeoutExpired) {
            this.startNewDataTimeout.delay(0.4, this);
    ...
    */
    applyFilters: function(searchText) {
        this.DataFilteredHash = this.AllCurrentData.clone();
        //if the text is not empty
        if (!Object.isEmpty(searchText)) {
            var elem;
            this.makeSearch = true;
            this.data.each(function(element) {
                elem = element.value[1]._object.index;
                if (!element.value[0].include(searchText)) {
                    //remove the elem in the data to show 
                    this.DataFilteredHash.unset(elem.split('_')[0]);
                }
            } .bind(this));
        }
        if (this.DataFilteredHash.keys().length == 0) {
            //show message 'no results found'
            this.virtualHtml.down('[id=tableContent]').hide();
            this.virtualHtml.down('[id=legend]').hide();
            this.virtualHtml.down('[id=tableContentEmpty]').show();
        } else {
            this.virtualHtml.down('[id=legend]').show();
            this.virtualHtml.down('[id=tableContent]').show();
            this.virtualHtml.down('[id=tableContentEmpty]').hide();
    ...
    */
    fillTable: function() {
        ...
        this.DataToShowHash.each(function(pair) {
            ...
            //create html structure
            body += "<tr id='row_" + id + '_' + appId + "'>" +
                        "<td><span id='link__" + id + '__' + appId + "' class='application_action_link selectorDummyClass'>" + datum + "</span><div id='req_" + id + "_" + appId + "' class='pdcPendReq_details' style='display:none'></div></td>" +
                        "<td class='application_main_text pdcPendReq_alignTop'>" + action + "</td>";
            if (status == '10' || status == '20' || status == '30') {
                body += "<td class='pdcPendReq_alignTop'><span class='application_icon_orange pdcPendReq_statusColumn'></span></td>";
            } else if (status == '21' || status == '31') {
                body += "<td class='pdcPendReq_alignTop'><span class='application_icon_red pdcPendReq_statusColumn'></span></td>";
            } else {
                body += "<td></td>";
            }
            body += "</tr>";
        ...
        if (!this.tableLoaded) {
            TableKit.Sortable.init(this.virtualHtml.down('table#pdcPendReq_table_' + this.tabId), {
                pages: global.paginationLimit
            });
            this.tableLoaded = true;
        } else {
            TableKit.reloadTable("pdcPendReq_table_" + this.tabId);
        }
        //create event in the table
        this.virtualHtml.down("[id=pdcPendReq_table_" + this.tabId + "]").observe("click", this.showHideDetails.bindAsEventListener(this));
    ...
    */
    showHideDetails: function(event) {
        ...
        if (element.match('span.selectorDummyClass')) {
            //get the info
            ...
            if (this.virtualHtml.down('[id=req_' + id + '_' + appId + ']')) {
                if (this.virtualHtml.down('[id=req_' + id + '_' + appId + ']').innerHTML == "") {
                    ...
                    if (!Object.isEmpty(json)) {
                        ...
                        this.virtualHtml.down('[id=req_' + id + '_' + appId + ']').insert(objFieldsPanel);
                    }
                }
                //show/hide details
