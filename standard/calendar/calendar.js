...
*/
{
    /**
     * @type {CAL_Calendar}
     * @description Object that contains the whole calendar logic so we can mantain it
    ...
    renderEmpty: function() {
        ...
        if (this.firstRun) {
            ...
            var prevMonth = new Element('input', {
                ...
                'class': 'application_verticalL_arrow application_handCursor',
                'type': 'button'
            });
            ...
            var neMonth = new Element('input', {
                'id': 'CAL_nextMonth',
                'class': 'application_verticalR_arrow application_handCursor',
                'type': 'button'
            });
            this.selectMonths.insert({
                after: neMonth
            ...
            this.controls.insert(new Element("div", {
    ...
    newEventGraphicObject: function(event) {
        ...
        for (var i = 0; i < numberOfWeeks; i++) {
            ...
            if (event.allDay) {
                ...
                if ((event.status == 4) || (event.status == 5) || (event.status == 6))
                    eventSegmentText += ' application_text_italic';
                eventSegmentText += '" title="' + event.begDate.toString('HH:mm') + " " + eventText + '">';
                eventSegmentText += event.begDate.toString('HH:mm') + " " + eventText;
                eventSegmentText += '</acronym></div>';
                eventSegmentText += '</div>';
                eventSegmentText += '</div>';
                eventSegmentText += '<div class="CAL_wholeDayEventBorder ' + color + '">';
            ...
            for (var j = 0; j < this.calendarMatrix[i].length; j++) {
                ...
                if ( flag <= 3 && flag != 0 ) {
                    for (var k = 0; k < 7; k++, date.add(1).day()) {
                        ...
                        if ( this.calendarMatrix[i][j][k] && 
                            this.calendarMatrix[i][j][k].id != lastDrawn) {
    ...
    onEmployeeSelected: function(event, unselect) {
        ...
        var jsonGetEvents = {
            EWS: {
                ...
                PARAM: {
                    o_begda_i: monthPreloadingData.begda.toString('yyyy-MM-dd'),
                    o_endda_i: monthPreloadingData.endda.toString('yyyy-MM-dd'),
 ...
 */
{
    ...
    getCalendarBounds: function(month) {
        ...
        if (typeof(month) == 'undefined') {
            ...
            var startingWeek = calendarBounds.begda.getWeek();
            var endingWeek = calendarBounds.endda.getWeek();
            this.numberOfWeeks = endingWeek - startingWeek;
