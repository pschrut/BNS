    ...
    _showReports: function(json, ID) {
        ...
        if (Object.jsonPathExists(json, 'EWS.o_result_list.yglui_str_rp_o_resultlist')) {
            ...
            for (var i = 0; i < repLength; i++) {
                ...
                var statusText = Object.isEmpty(this.statusLabels.get('Reporting_status_' + status)) ? "" : this.statusLabels.get('Reporting_status_' + status);
        ...
        if (this.reportLists == length) {
            var contentHTML = "<table id='FastEntryMenu_table' class='sortable'>" +
                                  "<thead class='application_header_barTable'>" +
                                      "<tr>" +
                                          "<th class='table_nosort'></th>" +
                                          "<th class='FastEntryMenu_table_textColumn'>" + global.getLabel('results') + "</th>" +
                                          "<th class='table_sortfirstdesc'>" + global.getLabel('date') + "</th>" +
                                          "<th></th>" +
                                      "</tr>" +
                                  "</thead>" +
            ...
            if (!this.tableCreated) {
                ...
                TableKit.reloadTable($(document.body).down("[id=FastEntryMenu_table]"));
        }
        else
    ...
    _showReportDetails: function(json, ID) {
        ...
        switch (type) {
            case 'WA_FE':
                ...
                // Tablekit
                if (creations) {
                    if (!this.creationTableCreated) {
                        TableKit.Sortable.init($(document.body).down("[id=FastEntryMenu_creationTable]"), { pages: parseInt(global.paginationLimit)/2 });
                        TableKit.options.autoLoad = false;
                        this.creationTableCreated = true;
                    }
                    else
                        TableKit.reloadTable($(document.body).down("[id=FastEntryMenu_creationTable]"));
                }
                if (errors) {
                    if (!this.errorTableCreated) {
                        TableKit.Sortable.init($(document.body).down("[id=FastEntryMenu_errorTable]"), { pages: parseInt(global.paginationLimit)/2 });
                        TableKit.options.autoLoad = false;
                        this.errorTableCreated = true;
                    }
                    else
                        TableKit.reloadTable($(document.body).down("[id=FastEntryMenu_errorTable]"));
                }
                break;
            case 'PCR_CDAT':
                ...
                infoMessage += "<br /><span>" + rows[length - 1].columns.yglui_str_rp_o_stresult_column['@value'] + "</span>";
                // Container
                var content = new Element('div');
                ...
                // Tablekit
                if (!Object.isEmpty(message)) {
                    if (!this.infotypeTableCreated) {
                        TableKit.Sortable.init($(document.body).down("[id=FastEntryMenu_infotypeTable]"), { pages: parseInt(global.paginationLimit)/2 });
                        TableKit.options.autoLoad = false;
                        this.infotypeTableCreated = true;
                    }
                    else
                        TableKit.reloadTable($(document.body).down("[id=FastEntryMenu_infotypeTable]"));
                }
                break;
            default:
                break;
