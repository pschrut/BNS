/** 
* @fileOverview manageHolderAssign.js 
* @description File containing class manageHolderAssign. 
* Application for Maintain and Display in OM.
*/

/**
*@constructor
*@description Class manageHolderAssign_standard.
*@augments getContentDisplayer 
*/
var ManageHolderAssign = Class.create(getContentDisplayer, {
    ...
    */
    run: function($super, args) {
        ...
        var buttonsHandlers = $H({
            DEFAULT_EVENT_THROW: 'EWS:manageHolderAssign_buttons_' + this.app
        });
        ...
        this.oType = this.parentType;
        $super(args);
        //buttons event
        this.onButtonBinding = this._actionButtonPressed.bindAsEventListener(this);
        document.observe('EWS:manageHolderAssign_buttons_' + this.app, this.onButtonBinding);
    },
    /*
    ...
    */
    _actionButtonPressed: function(event) {
        //get index of the first record
        var records = objectToArray(this.fp.json.EWS.o_field_values.yglui_str_wid_record);
        var minIndex = records[0].contents.yglui_str_wid_content['@rec_index'];
        //get values of necessary data
        this.action = getArgs(event).action;
        var recKey = getArgs(event).recKey - minIndex;
        ...
        switch (this.action) {
            case 'APP_OM_CANCEL': // cancel button
                ...
                break;
            case 'SCR_OM_ADDITION': // add occup (show the empty screen for an add occup)
                //get view
                var actions = objectToArray(this.fp.json.EWS.o_screen_buttons.yglui_str_wid_button);
                for (var i = 0; i < actions.length; i++) {
                    if (actions[i]['@action'] == "SCR_OM_ADDITION") {
                ...
                global.open($H({
                    ...
                    oType: this.parentType,
                    okCode: this.okCode
                }));
                break;
            case 'REC_OM_TRANSFER': // transfer button (save a transfer - row)
                this.saveScreen('transf', okCode, recKey, labelTag, record, screen);
                break;
            case 'REC_OM_DELETE': // remove assignm (row)
                this.deleteAssign(okCode, recKey, labelTag);
                break;
            case 'REC_OM_DELIMIT': // delimit assignm (row)
    ...
    */
    saveScreen: function(button, okCode, recKey, labelTag, record, screen) {
        //check the action of button
        if (button == 'add') {
            //service name for an additional occupancy
            this.saveRequestService = 'SAVE_ASSIGNM'
        } else {
            //service name for a transfer action
            this.saveRequestService = 'SAVE_TRANSF'
    ...
    */
    delimitRequest: function(code, recKey, labelTag) {
        //service name for delete or delimit action
        this.saveRequestService = 'SAVE_TRANSF'
        //build the json for the row
        var json = this.jsonToSave(recKey);
        this.json = json;
        ...
        for (var i = 0; i < fields.length; i++) {
            if (fields[i]['@fieldtechname'] == 'ENDDA') {
                this.json.EWS.o_field_values.yglui_str_wid_record.contents.yglui_str_wid_content.fields.yglui_str_wid_field[i]['@value'] = this.genericDelimitDatePicker.getActualDate().toString('yyyy-MM-dd');
            }
        }
        //save information
    ...
    */
    deleteRequest: function(code, recKey, labelTag) {
        //service name for delete action
        this.saveRequestService = 'SAVE_TRANSF'
        //build the json for the row
        var json = this.jsonToSave(recKey);
        this.json = json;
    ...
    */
    close: function($super) {
        $super();
        document.stopObserving('EWS:manageHolderAssign_buttons_' + this.app, this.onButtonBinding);
    }
});