...
var MyDocuments_admin = new Class.create(Application, {
    ...
    run: function($super, args) {
        ...
        switch (global.currentApplication.appId) {
            case 'DOC_C_AD': this.view = this.viewValues.Catalog; break;
            case 'DOC_L_AD': this.view = this.viewValues.List; break;
            case 'DOC_G_AD': this.view = this.viewValues.Grid; break;
            case 'DOC_F_AD': this.view = this.viewValues.CoverFlow; break;
            default:
    ...
    menuSync: function(event) {
        ...
        if (selected) {
            ...
            $('myDocuments_CurrentEmployee').update(global.getLabel('DML_CURRENTLY_SHOWING_DOCUMENTS_FROM') + ' : ' + this.empName);
            this.getMyDocuments();
            $('myDocuments_Header').show();
        }
    ...
    buildUI: function() {
        ...
        + '     <div style="float:left;">' + global.getLabel('DML_FIND_EMPLOYEE') + ' : </div><div id="myDocuments_employee" style="margin-left:10px;"></div>'
        + ' </div>';
        this.mainContainer.insert(html);
        this.buildHeader();
        ...
        this.empAutocompleter = new JSONAutocompleter('myDocuments_employee', {
    ...
    buildHeader: function() {
        ...
        var html = '<div id="myDocuments_Header" style="height:' + ((Prototype.Browser.IE) ? '58' : '88') + 'px;">'
        + '                 <div style="float:left;" id="myDocuments_CurrentEmployee"></div>'
        + '                 <div style="text-align:center;float:right;margin-right:220px;width:160px">'
        ...
        + '         <input type="text" id="myDocuments_Search" value="' + global.getLabel('DM_SEARCH') + '" class="application_autocompleter_box"/>'
...
+'  </tr>'
    ...
    buildDocumentMetaData: function(json) {
        ...
        + '             <div>'
			...
			+ '                 <div><b>' + global.getLabel('DML_DOCUMENT_PROPERTIES') + ':</b></div><br/>'
			+ '					<div style="float:left;vertical-align:middle;"><div style="float:left;">' + global.getLabel('DML_TYPE') + ' : </div><div id="myDocuments_Details_DocType" style="font-size:11px;float:right;"></div></div>'
			+ '                 <div style="float:left;vertical-align:middle;"><div style="float:left;">' + global.getLabel('DML_FILE_SIZE') + ' : </div><div id="myDocuments_Details_FileSize" style="font-size:11px;">' + fileSize + '</div></div>'
			+ '                 <div style="float:left;vertical-align:middle;"><div style="float:left;">' + global.getLabel('DML_STATUS') + ' : </div><div id="myDocuments_Details_Status" style="font-size:11px;float:right;">'
			+ '                     <input id="myDocuments_Details_StatusValidated" name="status" type="radio" ' + ((status.toLowerCase() == 'validated') ? 'checked="checked"' : '') + ' />' + global.getLabel('DML_VALIDATED') + ' &nbsp;&nbsp;'
			+ '                     <input id="myDocuments_Details_StatusUnvalidated" name="status" type="radio" ' + ((status.toLowerCase() == 'unvalidated') ? 'checked="checked"' : '') + ' />' + global.getLabel('DML_UNVALIDATED')
			+ '                 </div></div>'
			+ '                 <div style="float:left;vertical-align:middle;"><div style="float:left;">' + global.getLabel('DML_EMPLOYEE') + ' : </div><div id="myDocuments_Details_Employee" style="font-size:11px;float:right;"></div></div>'
			+ '                 <span style="float:left;vertical-align:middle;">' + global.getLabel('DML_SOURCE') + ' : ' + source + '</span><br/>'
			+ '                 <span style="float:left;vertical-align:middle;">' + global.getLabel('DML_CREATION_DATE') + ' : ' + creationDate + '</span><br/>'
			+ '                 <span style="float:left;vertical-align:middle;">' + global.getLabel('DML_MODIFICATION_DATE') + ' : ' + modificationData + '</span><br/>'
			+ '                 <span style="float:left;vertical-align:middle;">' + global.getLabel('DML_LAST_MODIFIED_BY') + ' : ' + lastModifiedBy + '</span><br/>'
			+ '                 <span style="float:left;vertical-align:middle;">' + global.getLabel('DML_ORIGNAL_FILE_NAME') + ' : ' + documentNameOrig + '</span><br/>'
			+ '                 <span style="float:left;vertical-align:middle;">' + global.getLabel('DML_TRACKING_ID') + ' : ' + documentTrackId + '</span><br/>'
			+ '             </div>'
			...
			+ '                 <textarea id="myDocuments_Details_Comments" style="width:98%;height:80px;font-size:11px;" class="application_autocompleter_box">' + comments + '</textarea><br>'
			+ '					<input type="hidden" id="myDocuments_Details_DocumentID" value="' + documentID + '">'
			+ '                 <div style="float:right;" id="myDocuments_SaveChanges">'
			+ '                     <div class="leftRoundedCorner">'
			+ '                         <span class="centerRoundedButton">' + global.getLabel('DML_SAVE_CHANGES') + '</span>'
			+ '                         <span class="rightRoundedCorner"></span>'
			+ '                     </div>'
			+ '                 </div>'
			+ '             </div>'
        ...
        if ((this.view == this.viewValues.Grid) || (this.view == this.viewValues.Catalog) || (this.view == this.viewValues.CoverFlow)) {
            ...
            this.detailspopUp.create();

        } else if ((this.view == this.viewValues.List) || (this.view == this.viewValues.CoverFlow)) {
        ...
        this.docTypeAutocompleter = new JSONAutocompleter('myDocuments_Details_DocType', {
            showEverythingOnButtonClick: true,
            timeout: 5000,
            templateResult: '#{text}',
            templateOptionsList: '#{text}',
        ...
        var items = this.employeesList.EWS.o_values.item;
        for (var i = 0; i < items.length; i++) {
            jsonObject.autocompleter.object.push({
                data: items[i]["@id"],
                text: items[i]["@value"]
        ...
        this.detailsEmployee = new JSONAutocompleter('myDocuments_Details_Employee', {
            showEverythingOnButtonClick: true,
            timeout: 5000,
            templateResult: '#{text}',
            templateOptionsList: '#{text}',
        ...
        if (this.emp != global.objectId) {
            this.detailsEmployee.setDefaultValue(this.emp, false, false);
        }
    ...
    buildDetailsTypeList: function(json) {
        var jsonObject = {
            autocompleter: {
                object: [],
                multilanguage: {
        ...
        this.docTypeAutocompleter.setDefaultValue(this.documentType, true, false);
    },
    ...
    registerEvents: function(documents) {
        ...
        // View Delete (bouton)
        $('myDocuments_Delete').observe('click', this.deleteDocuments.bind(this, documents));
    ...
    saveChanges: function() {
        ...
        var oldEmployee = this.empName;
        var newEmployee = this.detailsEmployee.getValue().textAdded;

        var oldEmpId = this.emp;
        var newEmpId = this.detailsEmployee.getValue().idAdded;

        var documentID = $('myDocuments_Details_DocumentID').value;
        var employee = this.detailsEmployee.getValue().idAdded;
        var documentType = this.docTypeAutocompleter.getValue().idAdded;
        var status = ($('myDocuments_Details_StatusValidated').checked) ? 'validated' : 'unvalidated';
        var documentComment = $('myDocuments_Details_Comments').value;

        var xmlin = ''
		+ ' <EWS>'
		+ ' 	<SERVICE>DM_UPD_METADATA</SERVICE>'
		+ ' 	<DEL/>'
		+ ' 	<PARAM>'
		+ ' 		<I_V_DOC_ID>' + documentID + '</I_V_DOC_ID>'
		+ ' 		<I_V_EMPLOYEE>' + employee + '</I_V_EMPLOYEE>'
		+ ' 		<I_V_DOC_TYPE>' + documentType + '</I_V_DOC_TYPE>'
		+ ' 		<I_V_STATUS>' + status + '</I_V_STATUS>'
        ...
        if (oldEmpId != newEmpId) {
            var html = ''
				+ '<div>'
				+ '<span class="application_main_error_text">' + global.getLabel('DML_DOCUMENT_MOVE_WARNING') + '</span><br/>'
				+ '<div>'
				+ global.getLabel('DML_BY_CHANGING_THE_EMPLOYEE')
				+ ' "' + documentName + '" '
				+ global.getLabel('DML_FROM_THE_EMPLOYEE_FILE_OF')
				+ ' "' + oldEmployee + '" '
				+ global.getLabel('DML_TO_THE_EMPLOYEE_FILE_OF')
				+ ' "' + newEmployee + '" '
				+ '<br/><br/>'
				+ global.getLabel('DML_ARE_YOU_SURE_YOU_WANT_TO_CONTINUE')
				+ '</div>'
				+ '</div>'
				+ '';
            var contentHTML = new Element('div', {}).update(html);
            var buttonsJson = {
                elements: [],
            ...
            buttonsJson.elements.push(btnYes);
            buttonsJson.elements.push(btnNo);
            ...
            var info = new infoPopUp({
                ...
                events: $H({ onClose: 'EWS:changeWarningClose' }),
                htmlContent: contentHTML,
                indicatorIcon: 'exclamation',
                width: 350
            });

            if ((this.view != this.viewValues.List) && this.detailspopUp) {
                this.detailspopUp.close();
                delete this.detailspopUp;
            }
