/**
 * @class
 * @description Class that manage the fiedls that should have an associated action
 * @author jonathanj & nicolasl
 * @version 1.1
 * <br/>Modification for version 1.1
 * <ul>
 * <li>Allow to have several display for the list of actions</li>
 * <li>Correct the condition to check if the cache is to use</li>
 * <li>Update the parameters used by the cache</li>
 * <li>Add the caller class name in the cache identifier</li>
 * <li>Bug fix, in the click on the user, the generated event was lost in IE</li>
 * </ul>
 */
var ScmUserAction = Class.create(/** @lends ScmUserAction.prototype */{
     ...
     * <ul>
     * <li>Update the condition to check if the caching is to use</li>
     * </ul>
     */
    addActionOnField: function(userId, userName, userCompany, marginTop, updateField, useCache) {
		...
		//since 1.1 Correct the condition (default value is true)
     ...
     * <br/>Modification for 1.1
     * <ul>
     * <li>Add the caller application in the cache item identifier</li>
     * <li>Create a clone of the event because in IE, it lead to problem</li>
     * </ul>
     */
    getListActions: function(event) {
        var cache;
		//since 1.1 Use the caller class as a parameter in the cache identifier
        ...
        if(Object.isEmpty(cache))
            this._caller.makeAJAXrequest($H({
				xml				: 	'<EWS>'
								...
								+		'</PARAM>'
								+	'</EWS>', 
				//since 1.1 Give a clone of the event as parameter
     ...
     * @since 1.0
     * <br/>Modified for version 1.1
     * <ul>
     * <li>Display the child method to display the menu</li>
     * <li>Add the caller application name in the cahe identifier</li>
     * <li>Limit the list of contextual actions to the authorized ones</li>
     * </ul>
     */
    displayListActions: function(event, listActionsJson) {
        //since 1.1 Some unused parameters were removed
        var listActions = $A();
		...
		//Set the list of contextual actions 
	   	if(listActionsJson.EWS.o_actions && listActionsJson.EWS.o_actions.yglui_vie_tty_ac){
			objectToArray(listActionsJson.EWS.o_actions.yglui_vie_tty_ac).each(function(action) {
				listActions.push({appId: action['@tarap'], view: action['@views'], tabId: (action['@tartb']||''), label: action['@actiot']});
			}.bind(this));
		}

		//Delete the prohibited actions from the list
		listActions = listActions.reject(function(action) {
			//since 1.1 Check if the application is loaded
			if(global.allApps.indexOf(action.appId) < 0) return true;
