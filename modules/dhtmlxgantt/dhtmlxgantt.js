...
{var task = this.getTaskById(id);if (task){if (!task.ParentTask){for (var i = 0;i < this.ParentTasks.length;i++){if (this.ParentTasks[i].Id == id){if (this.ParentTasks[i].nextParentTask){if (this.ParentTasks[i].previousParentTask){this.ParentTasks[i].previousParentTask.nextParentTask = this.ParentTasks[i].nextParentTask;this.ParentTasks[i].nextParentTask.previousParentTask = this.ParentTasks[i].previousParentTask}else {this.ParentTasks[i].nextParentTask.previousParentTask = null}}else {if (this.ParentTasks[i].previousParentTask){this.ParentTasks[i].previousParentTask.nextParentTask = null}};this.ParentTasks[i] = null;this.ParentTasks.splice(i, 1);break}}}else
 {var parentTask = task.ParentTask;for (var i = 0;i < parentTask.ChildTasks.length;i++){if (parentTask.ChildTasks[i].Id == id){if (parentTask.ChildTasks[i].nextChildTask){if (parentTask.ChildTasks[i].previousChildTask){parentTask.ChildTasks[i].previousChildTask.nextChildTask = parentTask.ChildTasks[i].nextChildTask;parentTask.ChildTasks[i].nextChildTask.previousChildTask = parentTask.ChildTasks[i].previousChildTask}else {parentTask.ChildTasks[i].nextChildTask.previousChildTask = null}}else {if (parentTask.ChildTasks[i].previousChildTask){parentTask.ChildTasks[i].previousChildTask.nextChildTask = null}};parentTask.ChildTasks[i] = null;parentTask.ChildTasks.splice(i, 1);break}}}}};GanttProjectInfo.prototype.addTask = function(task)
{this.ParentTasks.push(task);task.setProject(this)};

GanttProjectInfo.prototype.getTaskById = function(id)
{for (var j = 0;j < this.ParentTasks.length;j++){var task = this.getTaskByIdInTree(this.ParentTasks[j], id);if (task)return task};return null};GanttProjectInfo.prototype.getTaskByIdInTree = function(parentTask, id)
{if (parentTask.Id == id){return parentTask}else
 {for (var i = 0;i < parentTask.ChildTasks.length;i++){if (parentTask.ChildTasks[i].Id == id){return parentTask.ChildTasks[i]};if (parentTask.ChildTasks[i].ChildTasks.length > 0){if (parentTask.ChildTasks[i].ChildTasks.length > 0){var cTask = this.getTaskByIdInTree(parentTask.ChildTasks[i], id);if (cTask)return cTask}}}};return null};
 
 
 
 
 function GanttTaskInfo(id, name, est, duration, percentCompleted, predecessorTaskId, endda, realBegda, realEndda, status)
{this.endda = endda; 
 this.realBegda = realBegda;
 this.realEndda = realEndda;
 this.status = status;
...
{this.ChildTasks.push(task);task.ParentTask = this};

GanttTaskInfo.prototype.setProject = function(project)
{this.Project = project;for (var j = 0;j < this.ChildTasks.length;j++){this.ChildTasks[j].setProject(project)}};function GanttTask(taskInfo, project, chart)
{this.Chart = chart;this.Project = project;this.TaskInfo = taskInfo;this.checkMove = false;this.checkResize = false;this.moveChild = false;this.maxPosXMove = -1;this.minPosXMove = -1;this.maxWidthResize = -1;this.minWidthResize = -1;this.posX = 0;this.posY = 0;this.MouseX = 0;this.taskItemWidth = 0;this.isHide = false;this._heightHideTasks = 0;this._isOpen = true;this.descrTask = null;this.cTaskItem = null;this.cTaskNameItem = null;this.parentTask = null;this.predTask = null;this.childTask = [];this.childPredTask = [];this.nextChildTask = null;this.previousChildTask = null;this.nextParentTask = null;this.previousParentTask = null};function GanttProject(Chart,projectInfo)
{this.nextProject = null;this.previousProject = null;this.arrTasks = [];this.Project = projectInfo;this.Chart = Chart;this.percentCompleted = 0;this.Duration = 0;this.descrProject = null;this.projectItem = null;this.projectNameItem = null;this.posY = 0;this.posX = 0};GanttProject.prototype.checkWidthProjectNameItem = function()
{if (this.projectNameItem.offsetWidth + this.projectNameItem.offsetLeft > this.Chart.maxWidthPanelNames){var width = this.projectNameItem.offsetWidth + this.projectNameItem.offsetLeft - this.Chart.maxWidthPanelNames;var countChar = Math.round(width / (this.projectNameItem.offsetWidth / this.projectNameItem.firstChild.length));var pName = this.Project.Name.substring(0, this.projectNameItem.firstChild.length - countChar - 3);pName += "...";this.projectNameItem.innerHTML = pName}};GanttProject.prototype.create = function()
...
{if ((name != "")&& (name != null)) {this.Project.Name = name;if(this.Chart._showTreePanel){this.projectNameItem.innerHTML = name;this.projectNameItem.title = name;this.checkWidthProjectNameItem()};if(this.Chart.isShowDescProject)this.descrProject.innerHTML = this.getDescStr();this.addDayInPanelTime()}};
...
GanttProject.prototype.setPercentCompleted = function(percentCompleted){
	...
	if ((percentCompleted > 0)&& (percentCompleted < 100) && (this.percentCompleted > 0) && (this.percentCompleted < 100)){
		...
		if ((percentCompleted == 0)&& (this.percentCompleted == 100)){
			...
			this.projectItem[0].firstChild.rows[0].cells[0].firstChild.src = this.Chart.imgs + "progress_filled_success.png"}
		}else if (((percentCompleted > 0)|| (percentCompleted < 100)) && ((this.percentCompleted == 0) || (this.percentCompleted == 100))){
			this.projectItem[0].firstChild.rows[0].cells[0].parentNode.removeChild(this.projectItem[0].firstChild.rows[0].cells[0]);
			...
			imgPr.src = this.Chart.imgs + "progress_filled_success.png";
			...
			if (percentCompleted == 100){
				...
				var imgPr = document.createElement("img");
				imgPr.style.width = (percentCompleted * this.Duration * this.Chart.hourInPixelsWork) / 100 + "px";
				imgPr.style.height = this.Chart.heightTaskItem + "px";cellprojectItem.appendChild(imgPr);
				imgPr.src = this.Chart.imgs + "progress_filled_success.png";cellprojectItem = document.createElement("TD");
				this.projectItem[0].firstChild.rows[0].appendChild(cellprojectItem);
				cellprojectItem.width = (100 - percentCompleted) + "%";
				imgPr = document.createElement("img");
				imgPr.style.width = ((100 - percentCompleted) * this.Duration * this.Chart.hourInPixelsWork) / 100 + "px";
...
{var widthPred = this.getWidthOnDuration(predTask.Duration);var posPred = this.getPosOnDate(predTask.EST);var posChild = this.getPosOnDate(task.EST);if ((widthPred + posPred)> posChild)
 {return false};return true};
 
 GanttChart.prototype.correctPosPredecessorTask = function(predTask, ctask, ctaskObj)
{var newDate = new Date(predTask.EST);newDate.setHours(newDate.getHours() + (predTask.Duration/this.hoursInDay * 24))
 if (newDate.getHours()> 0){newDate.setHours(0);newDate.setDate(newDate.getDate()+1)};if (ctaskObj)ctaskObj.setEST(newDate, true);else ctask.EST = newDate;if(ctask.ParentTask){if(!this.checkPosParentTask(ctask.ParentTask,ctask)) 
 {var newDate2 = new Date(ctask.ParentTask.EST);newDate2.setHours(newDate2.getHours() + (ctask.ParentTask.Duration/this.hoursInDay * 24))
...
{var widthParent = this.getWidthOnDuration(parentTask.Duration);var posParent = this.getPosOnDate(parentTask.EST);var posChild = this.getPosOnDate(task.EST);var widthChild = this.getWidthOnDuration(task.Duration);if ((widthParent + posParent)< (posChild + widthChild)) {return false}else
 return true};
...
{if (!parentTask._isOpen){parentTask.cTaskNameItem[2].src = this.imgs + "minus.gif";parentTask._isOpen = true;parentTask.shiftCurrentTasks(parentTask, parentTask._heightHideTasks);parentTask.showChildTasks(parentTask, parentTask._isOpen);parentTask._heightHideTasks = 0}};GanttChart.prototype.getLastCloseParent = function(task){
	if (task.parentTask) {
		...
		(task.parentTask.cTaskNameItem[2].style.display == "none")) { return this.getLastCloseParent(task.parentTask)}  else { return task} }
 else { return task} 
 };
 ...
 GanttTask.prototype.setPercentCompleted = function(percentCompleted){
	...
	if ((percentCompleted != 0)&& (percentCompleted != 100)){
		if ((this.TaskInfo.PercentCompleted != 0)&& (this.TaskInfo.PercentCompleted != 100)){
			...
			var cellTblTask = document.createElement("td");this.cTaskItem[0].childNodes[0].firstChild.rows[0].appendChild(cellTblTask);
	...
	this.TaskInfo.PercentCompleted = percentCompleted;this.taskItemWidth = parseInt(this.cTaskItem[0].firstChild.firstChild.width);
...
{for (var i = 0;i < this.Project.length;i++){if (this.Project[i].Id == id){return this.Project[i]}};return null};GanttChart.prototype.getPopUpTime = function(event)
{var cellTime = event.target || event.srcElement;var posY = cellTime.offsetParent.offsetParent.offsetParent.offsetTop - 25;var posX = cellTime.offsetLeft - cellTime.offsetParent.offsetParent.offsetParent.scrollLeft;this.divTimeInfo.style.cssText = "position:absolute;display: inline;top: " + posY + "px;left: " + posX + "px;";var tblTimeInfo = this.divTimeInfo.firstChild;var date = new Date(this.startDate);date.setDate(date.getDate() + parseInt(cellTime.innerHTML));tblTimeInfo.rows[0].cells[0].innerHTML = "<span class='st'>" + date.getDate() + "." + (date.getMonth() + 1) + "." + date.getFullYear() + "</span>"};GanttChart.prototype.closePopUpTime = function(event)
{this.divTimeInfo.style.cssText = "display: none"};

GanttChart.prototype.loadData = function(content,isFile,isLocal)
{this.clearAll();if((isFile==null)||(isFile=='undefined'))
 {isFile = false};if((isLocal==null)||(isLocal=='undefined'))
 {isLocal = false};this.loadXML(content,isFile,isLocal);this.Project.sort(this.sort_byStartDate);this.startDate = this.getStartDate();for (var i = 0;i < this.Project.length;i++){for (var k = 0;k < this.Project[i].ParentTasks.length;k++){if ((this.Project[i].ParentTasks[k].EST != null)&& (this.Project[i].ParentTasks[k].EST != '')) {this.setESTChild(this.Project[i].ParentTasks[k])}else {this.Error.throwError("DATA_ERROR", 25, [this.Project[i].ParentTasks[k].Id]);return};if (this.setPredTask(this.Project[i])) return};for (var k = 0;k < this.Project[i].ParentTasks.length;k++){if (this.Project[i].ParentTasks[k].EST < this.Project[i].StartDate){this.Error.throwError("DATA_ERROR", 24, [this.Project[i].ParentTasks[k].Id,this.Project[i].Id]);return};if (this.checkPosParentTaskInTree(this.Project[i].ParentTasks[k])) return};this.sortTasksByEST(this.Project[i])};for (var i = 0;i < this.Project.length;i++){var project = new GanttProject(this,this.Project[i]);if (this.arrProjects.length > 0){var previousProject = this.arrProjects[this.arrProjects.length - 1];project.previousProject = previousProject;previousProject.nextProject = project};project.create();this.checkHeighPanelTasks();this.arrProjects.push(project);this.createTasks(project)}};GanttChart.prototype.clearAll = function()
...
{if (isFile && (content == null || content == ""))
 ...
 if(!isLocal){this.xmlLoader.loadXML(this.loadPath + "?path=" + content + "&rnd=" + (new Date() - 0), "post")}else
 {this.xmlLoader.loadXML(content,"post")};this.doLoadDetails(isLocal)}catch(e)
...
{if (a.EST < b.EST)return -1;if (a.EST > b.EST)return 1;return 0};

GanttChart.prototype.sort_byStartDate = function(a, b)
{if (a["StartDate"] < b["StartDate"])return -1;if (a["StartDate"] > b["StartDate"])return 1;return 0};
...
{var divTasks = document.createElement("div");divTasks.className = "taskPanel";divTasks.style.cssText = "position:relative;";divTasks.style.height = this.contentHeight - 63 + "px";divTasks.style.background = "url(" + this.imgs + "bg.png)";return divTasks};GanttChart.prototype.createPanelNamesTasks = function()
{var divListNames = document.createElement("div");divListNames.innerHTML = "&nbsp;";divListNames.style.cssText = "position:relative;background:url(" + this.imgs + "bg.png)";divListNames.style.height = this.contentHeight - 63 + "px";divListNames.style.width = this.maxWidthPanelNames + "px";return divListNames};

GanttChart.prototype.createPopUpInfo = function()
{var divTaskInfo = document.createElement("div");divTaskInfo.style.cssText = 'display: none;';var tblTaskInfo = document.createElement("table");tblTaskInfo.style.cssText = "position:absolute;top:0px;left:0px; width:200px;";tblTaskInfo.className = "poPupInfo";divTaskInfo.appendChild(tblTaskInfo);var rowTaskInfo = tblTaskInfo.insertRow(tblTaskInfo.rows.length);var cellTaskInfo = document.createElement("td");rowTaskInfo.appendChild(cellTaskInfo);return divTaskInfo};GanttChart.prototype.createPopUpTimeInfo = function()
{var divTimeInfo = document.createElement("div");divTimeInfo.style.cssText = 'display: none;';var tblTimeInfo = document.createElement("table");tblTimeInfo.className = "poPupTime";divTimeInfo.appendChild(tblTimeInfo);var rowTimeInfo = tblTimeInfo.insertRow(tblTimeInfo.rows.length);var cellTimeInfo = document.createElement("td");cellTimeInfo.align = "center";rowTimeInfo.appendChild(cellTimeInfo);return divTimeInfo};GanttChart.prototype.createPanelTime = function()
{var panelTime = document.createElement("div");panelTime.style.cssText = "position:relative";var tblTime = document.createElement("table");panelTime.appendChild(tblTime);tblTime.cellPadding = "0px";tblTime.border = "0px";tblTime.cellSpacing = "0px";tblTime.bgColor = "#FFFFFF";tblTime.style.cssText = ";cursor: pointer;margin-top: 0px";var newRow = tblTime.insertRow(tblTime.rows.length);for (var i = 0;i < this.countDays;i++){this.addPointInTimePanel(newRow, panelTime);this.addDayInPanelTime(newRow)};return panelTime};GanttChart.prototype.addPointInTimePanel = function(row, panelTime)
{var smallLine = document.createElement("div");var leftLine = document.createElement("div");smallLine.style.cssText = "position:absolute;left:" + ( row.cells.length * this.dayInPixels + 12 ) + "px;top:" + 0 + "px;height:5px;width:1px;font-size: 1px;margin-left:0px;margin-right:0px;margin-top:0px;margin-bottom:0px;background:#858585;";leftLine.style.cssText = "position:absolute;left:" + ( row.cells.length * this.dayInPixels ) + "px;top:" + 0 + "px;height:7px;width:1px;font-size: 1px;margin-left:0px;margin-right:0px;margin-top:0px;margin-bottom:0px;background:#858585;";panelTime.appendChild(smallLine);panelTime.appendChild(leftLine)};GanttChart.prototype.addDayInPanelTime = function(row)
{var self = this;var newCell = document.createElement("TD");newCell.style.height = "30px";newCell.style.width = this.dayInPixels + "px";newCell.align = "center";newCell.vAlign = "middle";newCell.className = "dayNumber";newCell.innerHTML = row.cells.length;if ((this._isFF)|| (this._isOpera))
 {var getPopUpTime = function(event) {self.getPopUpTime(event)};var closePopUpTime = function(event) {self.closePopUpTime(event)};newCell.onmouseover = getPopUpTime;newCell.onmouseout = closePopUpTime}else if (this._isIE){var getPopUpTime = function() {self.getPopUpTime(event)};var closePopUpTime = function() {self.closePopUpTime(event)};newCell.onmouseover = getPopUpTime;newCell.onmouseout = closePopUpTime};row.appendChild(newCell)};GanttChart.prototype.incHeightPanelTasks = function(height)
...
{var self = this;var content = document.getElementById(divId);this.content = content;this.getBrowserType();if (this._isIE){document.body.attachEvent('onselectstart', function() {window.event.returnValue = false});document.body.attachEvent('onkeydown', function() {if (event.keyCode == 65 && event.ctrlKey)window.event.returnValue = false})}else {content.addEventListener('mousedown', function(e) {e.preventDefault()}, true);document.addEventListener('keydown', function(e) {if (e.keyCode == 65 && e.ctrlKey)e.preventDefault()}, true)};this.Error.catchError("DATA_ERROR", function(type, descr, params) {self.errorDataHandler(type, descr, params)
 ...
 });var tableControl = document.createElement("table");tableControl.cellPadding = "0";tableControl.cellSpacing = "0";tableControl.style.cssText = "width: 100%;position: relative;";var newRowTblControl = tableControl.insertRow(tableControl.rows.length);var newCellTblControl;this.contentHeight = content.offsetHeight;this.contentWidth = content.offsetWidth;content.appendChild(tableControl);this.countDays = this.getCountDays();this.panelTime = document.createElement("div");this.panelTime.appendChild(this.createPanelTime());this.panelTime.style.cssText = "position:relative;overflow:hidden;height:34px";this.oData = document.createElement("div");this.oData.appendChild(this.createPanelTasks());this.oData.style.cssText = "position:relative;overflow:scroll;height:" + (this.contentHeight - 40) + "px;border-left:#f1f3f1 1px solid";this.oData.firstChild.appendChild(this.createPanelErrors());if (this._showTreePanel){this.panelNames = document.createElement("div");newCellTblControl = document.createElement("td");newCellTblControl.vAlign = "top";this.panelNames.appendChild(this.createPanelNamesTasks());this.panelNames.style.cssText = "position:relative;overflow:hidden;border-left:#f1f3f1 1px solid;border-bottom:#f1f3f1 1px solid";newCellTblControl.appendChild(this.panelNames);newRowTblControl.appendChild(newCellTblControl)};newCellTblControl = document.createElement("td");var divCell = document.createElement("div");divCell.style.cssText = "position: relative;";divCell.appendChild(this.oData);divCell.appendChild(this.panelTime);newCellTblControl.appendChild(divCell);newRowTblControl.appendChild(newCellTblControl);if (this._showTreePanel){this.panelNames.style.height = (this.contentHeight - 56) + "px";this.panelNames.style.width = this.maxWidthPanelNames + "px";this.oData.style.width = (this.contentWidth - this.maxWidthPanelNames) + "px";this.oData.firstChild.style.width = this.dayInPixels * this.countDays + "px";this.panelTime.style.width = (this.contentWidth - this.maxWidthPanelNames - 18) + "px";this.panelTime.firstChild.style.width = this.dayInPixels * this.countDays + "px";if(this.isShowConMenu)this.contextMenu = new contextMenu(this)}else {this.oData.style.width = this.contentWidth + "px";this.panelTime.style.width = (this.contentWidth - 16) + "px"};if(this._isOpera){this.oData.onmousewheel = function(e) {return false}};this.oData.onscroll = function() {self.panelTime.scrollLeft = this.scrollLeft;if (self.panelNames){self.panelNames.scrollTop = this.scrollTop;if(self.isShowConMenu)self.contextMenu.hide()}};this.divTimeInfo = this.createPopUpTimeInfo();divCell.appendChild(this.divTimeInfo);this.divInfo = this.createPopUpInfo();this.oData.firstChild.appendChild(this.divInfo);this.Project.sort(this.sort_byStartDate);this.startDate = this.getStartDate();for (var i = 0;i < this.Project.length;i++){for (var k = 0;k < this.Project[i].ParentTasks.length;k++){if (this.isEmpty(this.Project[i].ParentTasks[k].EST)) {this.Project[i].ParentTasks[k].EST = this.Project[i].StartDate};this.setESTChild(this.Project[i].ParentTasks[k]);if (this.setPredTask(this.Project[i])) return};for (var k = 0;k < this.Project[i].ParentTasks.length;k++){if (this.Project[i].ParentTasks[k].EST < this.Project[i].StartDate){if(!this.correctError){this.Error.throwError("DATA_ERROR", 24, [this.Project[i].ParentTasks[k].Id,this.Project[i].Id]);return}else{this.Project[i].ParentTasks[k].EST = this.Project[i].StartDate}};if (this.checkPosParentTaskInTree(this.Project[i].ParentTasks[k])) return};this.sortTasksByEST(this.Project[i])};for (var i = 0;i < this.Project.length;i++){var project = new GanttProject(this,this.Project[i]);if (this.arrProjects.length > 0){var previousProject = this.arrProjects[this.arrProjects.length - 1];project.previousProject = previousProject;previousProject.nextProject = project};project.create();this.checkHeighPanelTasks();this.arrProjects.push(project);this.createTasks(project)};return this};GanttChart.prototype.isEmpty = function(value)
{if(value == null || value == '')return true;return false};
...
{var posY = object.offsetTop + this.Chart.heightTaskItem + 6;var posX = object.offsetLeft + ((event.layerX == null) ? event.offsetX : event.layerX);var tblInfo = this.Chart.divInfo.lastChild;


tblInfo.rows[0].cells[0].innerHTML = "<div style='font-size: 12px;font-weight: bold; margin: 0px 0px 4px 0px;'>" + this.TaskInfo.Name + "</div>";
tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Begin Date:&nbsp;</span><span class='ut'>" + this.TaskInfo.realBegda.getDate() + "." + (this.TaskInfo.realBegda.getMonth() + 1) + "." + this.TaskInfo.realBegda.getFullYear() + "</span><br/>";
tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>End date:&nbsp;</span><span class='ut'>" + this.TaskInfo.realEndda.getDate() + "." + (this.TaskInfo.realEndda.getMonth() + 1) + "." + this.TaskInfo.realEndda.getFullYear() + "</span><br/>";
/*
...
if (this.predTask){tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Predecessor Task:&nbsp;</span>";tblInfo.rows[0].cells[0].innerHTML += "<span class='lt'>*" + this.TaskInfo.PredecessorTask.Name + "</span>"};
if (this.TaskInfo.ChildTasks.length != 0){tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Child Tasks:&nbsp;</span>";for (var i = 0;i < this.TaskInfo.ChildTasks.length;i++){tblInfo.rows[0].cells[0].innerHTML += (i == this.TaskInfo.ChildTasks.length - 1) ? ("<span class='lt'>*" + this.TaskInfo.ChildTasks[i].Name + "</span>") : ("<span class='lt'>*" + this.TaskInfo.ChildTasks[i].Name + "</span>")}};
...
*/
this.Chart.divInfo.style.cssText = "z-index:2;position: absolute;display: inline;";if (posY + this.Chart.divInfo.lastChild.offsetHeight + 10 > this.Chart.oData.offsetHeight + this.Chart.oData.scrollTop){this.Chart.divInfo.style.top = (posY - this.Chart.divInfo.lastChild.offsetHeight - 10 - this.Chart.heightTaskItem) + "px"}else {this.Chart.divInfo.style.top = posY + "px"};if (this.Chart.divInfo.lastChild.offsetWidth + posX + 10 > this.Chart.oData.offsetWidth + this.Chart.oData.scrollLeft){this.Chart.divInfo.style.left = posX - (this.Chart.divInfo.lastChild.offsetWidth + posX + 20 - (this.Chart.oData.offsetWidth + this.Chart.oData.scrollLeft)) + "px"}else {this.Chart.divInfo.style.left = posX + "px"}};GanttTask.prototype.closePopUpInfo = function (event)


{this.Chart.divInfo.style.cssText = "display: none"};

GanttTask.prototype.createConnectingLinesPN = function()
{var arrConnectingLinesNames = [];var lineVerticalLeft = document.createElement("div");lineVerticalLeft.style.cssText = "border-width: 0px 0px 0px 1px;border-style: dotted;border-color: #C0C4C0;margin: 0px;padding: 0px;z-index:10;position: absolute;" +
...
{this.shiftNextTask(this,height);task.Project.shiftNextProject(task.Project,height)};
...
{if (this.Chart._showTreePanel){task.cTaskNameItem[0].style.top = parseInt(task.cTaskNameItem[0].style.top) + height + "px";if (task.cTaskNameItem[2]){task.cTaskNameItem[2].style.top = parseInt(task.cTaskNameItem[2].style.top) + height + "px"};if(task.parentTask){if ((parseInt(this.cTaskNameItem[0].style.top)> parseInt(task.parentTask.cTaskNameItem[0].style.top))
 && (task.cTaskNameItem[1][0].style.display != "none")) {task.cTaskNameItem[1][0].style.height = parseInt(task.cTaskNameItem[1][0].style.height) + height + "px"}else {task.cTaskNameItem[1][0].style.top = parseInt(task.cTaskNameItem[1][0].style.top) + height + "px"};task.cTaskNameItem[1][1].style.top = parseInt(task.cTaskNameItem[1][1].style.top) + height + "px"}};task.cTaskItem[0].style.top = parseInt(task.cTaskItem[0].style.top) + height + "px";if(this.Chart.isShowDescTask){task.descrTask.style.top = parseInt(task.descrTask.style.top) + height + "px"};if(task.predTask){if (((parseInt(this.cTaskItem[0].style.top)> parseInt(task.predTask.cTaskItem[0].style.top)) ||
...
{var width = parseInt(this.projectItem[0].style.left) + parseInt(this.projectItem[0].firstChild.style.width) + 20;if(this.Chart.isShowDescProject){width += this.descrProject.offsetWidth};if (parseInt(this.Chart.panelTime.firstChild.firstChild.offsetWidth)< width)
 {var countDays = Math.round(width - parseInt(this.Chart.panelTime.firstChild.firstChild.offsetWidth)) / this.Chart.dayInPixels;for (var n = 0;n < countDays;n++){this.Chart.addPointInTimePanel(this.Chart.panelTime.firstChild.firstChild.rows[0], this.Chart.panelTime.firstChild);this.Chart.addDayInPanelTime(this.Chart.panelTime.firstChild.firstChild.rows[0])};this.Chart.panelTime.firstChild.firstChild.style.width = this.Chart.dayInPixels * (this.Chart.panelTime.firstChild.firstChild.rows[0].cells.length) + "px";this.Chart.oData.firstChild.style.width = this.Chart.dayInPixels * (this.Chart.panelTime.firstChild.firstChild.rows[0].cells.length) + "px"}};GanttProject.prototype.addEvent = function (elm, evType, fn, useCapture)
{if (elm.addEventListener){elm.addEventListener(evType, fn, useCapture);return true}else if (elm.attachEvent){var r = elm.attachEvent('on' + evType, fn);return r}else {elm['on' + evType] = fn}};GanttProject.prototype.getPopUpInfo = function(object,event)
{var posX = object.offsetLeft + ((event.layerX == null) ? event.offsetX : event.layerX);var posY = object.offsetTop + this.Chart.heightTaskItem + 6;var tblInfo = this.Chart.divInfo.lastChild;tblInfo.rows[0].cells[0].innerHTML = "<div style='font-family: Arial, Helvetica, Sans-serif;font-size: 12px;font-weight: bold;color: #688060;margin: 0px 0px 4px 0px;'>" + this.Project.Name + "</div>";tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Start Date:&nbsp;</span><span class='ut'>" + this.Project.StartDate.getDate() + "." + (this.Project.StartDate.getMonth() + 1) + "." + this.Project.StartDate.getFullYear() + "</span><br/>";tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Duration:&nbsp;</span><span class='ut'>" + this.Duration + " hours</span><br/>";tblInfo.rows[0].cells[0].innerHTML += "<span class='st'>Percent Complete:&nbsp;</span><span class='ut'>" + this.percentCompleted + "%</span><br/>";this.Chart.divInfo.style.cssText = "z-index:2;position: absolute;display: inline;";if (posY + this.Chart.divInfo.lastChild.offsetHeight + 6 > this.Chart.oData.offsetHeight + this.Chart.oData.scrollTop){this.Chart.divInfo.style.top = (posY - this.Chart.divInfo.lastChild.offsetHeight - 10 - this.Chart.heightTaskItem) + "px"}else {this.Chart.divInfo.style.top = posY + "px"};if (this.Chart.divInfo.lastChild.offsetWidth + posX + 10 > this.Chart.oData.offsetWidth + this.Chart.oData.scrollLeft){this.Chart.divInfo.style.left = posX - (this.Chart.divInfo.lastChild.offsetWidth + posX + 20 - (this.Chart.oData.offsetWidth + this.Chart.oData.scrollLeft)) + "px"}else {this.Chart.divInfo.style.left = posX + "px"}};GanttProject.prototype.closePopUpInfo = function (event)
{this.Chart.divInfo.style.cssText = "display: none"};
...
{var taskItem = this.cTaskItem[0];var width = parseInt(taskItem.style.left) + parseInt(taskItem.firstChild.firstChild.width) + 20;if(this.Chart.isShowDescTask){width += this.descrTask.offsetWidth};if (parseInt(this.Chart.panelTime.firstChild.firstChild.offsetWidth)< width)
 {var countDays = Math.round((width + 20 - parseInt(this.Chart.panelTime.firstChild.firstChild.offsetWidth)) / this.Chart.dayInPixels);for (var n = 0;n < countDays;n++){this.Chart.addPointInTimePanel(this.Chart.panelTime.firstChild.firstChild.rows[0], this.Chart.panelTime.firstChild);this.Chart.addDayInPanelTime(this.Chart.panelTime.firstChild.firstChild.rows[0])};this.Chart.panelTime.firstChild.firstChild.style.width = this.Chart.dayInPixels * (this.Chart.panelTime.firstChild.firstChild.rows[0].cells.length) + "px";this.Chart.oData.firstChild.style.width = this.Chart.dayInPixels * (this.Chart.panelTime.firstChild.firstChild.rows[0].cells.length) + "px"}};GanttTask.prototype.getDateOnPosition = function(position)
...
{var pageX = event.screenX;var posTaskItem = (this.posX + (pageX - this.MouseX));var widthTaskItem = parseInt(this.cTaskItem[0].childNodes[0].firstChild.width);var posTaskItemR = posTaskItem + widthTaskItem;if (this.checkMove){if (((this.minPosXMove <= posTaskItem))
...
{this.addDayInPanelTime();this.cTaskItem[0].style.left = posX + "px";this.cTaskItem[0].childNodes[1].firstChild.rows[0].cells[0].innerHTML = this.getDateOnPosition(posX).getDate() +'.'+ (this.getDateOnPosition(posX).getMonth()+1)+ '.' + this.getDateOnPosition(posX).getUTCFullYear()};GanttTask.prototype.resizeItem = function(event)
{if (this.checkResize){var taskItem = this.cTaskItem[0];var MouseX = event.screenX;var width = (MouseX - this.MouseX);var widthTaskItem = this.taskItemWidth + (MouseX - this.MouseX);if (widthTaskItem >= this.taskItemWidth){if ((widthTaskItem <= this.maxWidthResize)|| (this.maxWidthResize == -1))
...
{var taskItem = this.cTaskItem[0];var countHours = Math.round(width / this.Chart.hourInPixelsWork);if (taskItem.childNodes[0].firstChild.rows[0].cells[0]){taskItem.childNodes[0].firstChild.rows[0].cells[0].firstChild.style.width = parseInt(taskItem.childNodes[0].firstChild.rows[0].cells[0].width) * width / 100 + "px"};if (taskItem.childNodes[0].firstChild.rows[0].cells[1]){taskItem.childNodes[0].firstChild.rows[0].cells[1].firstChild.style.width = parseInt(taskItem.childNodes[0].firstChild.rows[0].cells[1].width) * width / 100 + "px"};taskItem.childNodes[0].firstChild.width = width + "px";taskItem.childNodes[1].firstChild.width = width + "px";this.cTaskItem[0].childNodes[1].firstChild.rows[0].cells[0].innerHTML = countHours;taskItem.childNodes[2].childNodes[0].style.width = width + "px";taskItem.childNodes[2].childNodes[1].style.left = width - 10 + "px"};GanttTask.prototype.endResizeItem = function()
{var taskItem = this.cTaskItem[0];if ((this.taskItemWidth != parseInt(taskItem.childNodes[0].firstChild.width)))
 {var posXL = taskItem.offsetLeft;var posXR = taskItem.offsetLeft + parseInt(taskItem.childNodes[0].firstChild.width);var countHours = Math.round((posXR - posXL) / this.Chart.hourInPixelsWork);this.TaskInfo.Duration = countHours;if (this.childPredTask.length > 0){for (var j = 0;j < this.childPredTask.length;j++){this.childPredTask[j].cTaskItem[1][2].style.width = parseInt(this.childPredTask[j].cTaskItem[1][2].style.width) - (parseInt(taskItem.childNodes[0].firstChild.width) - this.taskItemWidth) + "px";this.childPredTask[j].cTaskItem[1][2].style.left = parseInt(this.childPredTask[j].cTaskItem[1][2].style.left) + (parseInt(taskItem.childNodes[0].firstChild.width) - this.taskItemWidth) + "px";this.childPredTask[j].cTaskItem[1][0].style.left = parseInt(this.childPredTask[j].cTaskItem[1][0].style.left) + (parseInt(taskItem.childNodes[0].firstChild.width) - this.taskItemWidth) + "px"}}};this.cTaskItem[0].childNodes[1].firstChild.rows[0].cells[0].innerHTML = "";this.checkResize = false;this.taskItemWidth = 0;this.MouseX = 0;if(this.Chart.isShowDescTask){this.showDescTask()};this.Project.shiftProjectItem();if (this.Chart._isFF)document.body.style.cursor = ""};GanttProject.prototype.moveDescrProject = function()
{var posX = (parseInt(this.projectItem[0].style.left) + this.Duration * this.Chart.hourInPixelsWork + 10);this.descrProject.style.left = posX;this.descrProject.innerHTML = this.getDescStr()};GanttProject.prototype.showDescrProject = function()
...
{var str = '', delim = ", ";for(var i=0;i<this.Chart.paramShowProject.length;i++){switch (this.Chart.paramShowProject[i]) {case "Name":
 ...
 if(str != "")str += delim;str += this.Project[this.Chart.paramShowProject[i]].getDate() +"." + (this.Project[this.Chart.paramShowProject[i]].getMonth() + 1) + "." + this.Project[this.Chart.paramShowProject[i]].getFullYear();break;case "Duration":
     ...
     if ((this.Chart._isFF) || (this.Chart._isOpera)) {
         var getPopUpInfo = function(event){
             if ((!self.Chart._isMove) && (!self.Chart._isResize)) 
                 self.getPopUpInfo(self.descrProject, event)
         };
 ...
 GanttProject.prototype.createProjectItem = function(){
     ...
     if ((this.Chart._isFF) || (this.Chart._isOpera)) {
         var getPopUpInfo = function(event){
             if ((!self.Chart._isMove) && (!self.Chart._isResize)) 
                 self.getPopUpInfo(self.projectItem[0], event)
         };
         var closePopUpInfo = function(event){
             self.closePopUpInfo(event)
         };
 ...
 GanttProject.prototype.createProjectNameItem = function(){
     ...
     divName.style.cssText = "cursor:pointer;font-weight: bold;font-size: 12px;white-space:nowrap;height:15px;z-index:1;position: absolute;left:" + 5 + "px;top: " + this.posY + "px;";
     divName.innerHTML = this.Project.Name;
     divName.title = this.Project.Name.stripTags();
     if (this.Chart.isShowConMenu) {
         ...
         if (this.Chart._isIE) {
             this.addEvent(divName, "contextmenu", function(){
                 return false
             }, false)
 ...
 GanttProject.prototype.getPercentCompleted = function(){
     ...
     if (this.Project.ParentTasks.length != 0) { return percentCompleted = Math.round(sum / this.Project.ParentTasks.length)}  else { return percentCompleted = -1} };
...
{if (elm.addEventListener){elm.addEventListener(evType, fn, useCapture);return true}else if (elm.attachEvent){var r = elm.attachEvent('on' + evType, fn);return r}else {elm['on' + evType] = fn}};GanttTask.prototype.startMove = function (event)
...
{var str = '', delim = ", ";for(var i=0;i<this.Chart.paramShowTask.length;i++){switch (this.Chart.paramShowTask[i]) {case "Name":
 if(str != "")str += delim;str += this.TaskInfo[this.Chart.paramShowTask[i]];break;case "EST":
 if(str != "")str += delim;str += this.TaskInfo[this.Chart.paramShowTask[i]].getDate() +"." + (this.TaskInfo[this.Chart.paramShowTask[i]].getMonth() + 1) + "." + this.TaskInfo[this.Chart.paramShowTask[i]].getFullYear();break;case "Duration":
 if(str != "")str += delim;str += this.TaskInfo[this.Chart.paramShowTask[i]] + "h";break;case "PercentCompleted":
 if(str != "")str += delim;str += this.TaskInfo[this.Chart.paramShowTask[i]] + "%";break;default:
 break}};return str};
...
{var taskItem = this.cTaskItem[0];var posParentTaskItem = (this.parentTask == null) ? 0 : parseInt(this.parentTask.cTaskItem[0].style.left);var widthParentTaskItem = (this.parentTask == null) ? 0 : parseInt(this.parentTask.cTaskItem[0].childNodes[0].firstChild.width);var posTaskItem = parseInt(this.cTaskItem[0].style.left);var childPredPosX = 0;var childParentPosX = 0;if (this.childPredTask.length > 0){var posChildTaskItem = null;for (var n = 0;n < this.childPredTask.length;n++){if ((!posChildTaskItem)|| ((posChildTaskItem) && (posChildTaskItem > parseInt(this.childPredTask[n].cTaskItem[0].style.left))))
 ...
 GanttTask.prototype.createTaskItem = function(){
     ...
     if (this.TaskInfo.PercentCompleted != 0) {
		 ...
		 var imageToDisplay = 'progress_filled.png';
		 switch(self.TaskInfo.status){
		 	case 2: // in progress
		 				break;
			case 6: // pending			
			case 7: // pending
						break;
			case 3: // success
						imageToDisplay = 'progress_filled_success.png';
						break;
			case 5: // failed
						imageToDisplay = 'progress_filled_failed.png';
						break;
			case 1: // planned
						break;
			case 4: // cancelled
         ...
         imgPr.src = this.Chart.imgs + imageToDisplay;
     };
     if (this.TaskInfo.PercentCompleted != 100) {
         ...
         imgPrF.style.height = this.Chart.heightTaskItem + "px";
         cellTblTask.appendChild(imgPrF);
		 var imageToDisplay = 'progress_bg.png';
		 switch(self.TaskInfo.status){
		 	case 2: // in progress
		 				break;
			case 6: // pending
			...
			case 7: // pending
						imageToDisplay = 'progress_bg_pending.png';
						break;
			case 3: // success
						break;
			case 5: // failed
						...
						break;
			case 1: // planned
						imageToDisplay = 'progress_bg_planned.png';
						break;
			case 4: // cancelled
						break; 
		 }
         imgPrF.src = this.Chart.imgs + imageToDisplay;
     };
     ...
     if ((this.Chart._isFF) || (this.Chart._isOpera)) {
         var getPopUpInfo = function(event){
             if ((!self.Chart._isMove) && (!self.Chart._isResize)) 
                 self.getPopUpInfo(self.cTaskItem[0], event)
         };
         var closePopUpInfo = function(event){
             self.closePopUpInfo(event)
         };
 ...
 GanttTask.prototype.createTaskDescItem = function(){
     ...
     if ((this.Chart._isFF) || (this.Chart._isOpera)) {
         var getPopUpInfo = function(event){
             if ((!self.Chart._isMove) && (!self.Chart._isResize)) 
                 self.getPopUpInfo(self.descrTask, event)
         };
         var closePopUpInfo = function(event){
             self.closePopUpInfo(event)
         };
 ...
 GanttChart.prototype.getLastChildTask = function(task){
     if (task.childTask.length > 0) { return this.getLastChildTask(task.childTask[task.childTask.length - 1])}  else { return task} };
 ...
 };function GanttError() {this.catches = [];this._errors = [];this._init();return this};GanttError.prototype._init = function()
{
...
GanttError.prototype.catchError = function(type, handler) {this.catches[type] = handler};GanttError.prototype.getErrorString = function(str, params)
...
{var tab = new contextMenuTab(id,desc,type,showOInfo,menu);this.arrTabs.push(tab);return tab};
...
{if (elm.addEventListener){elm.addEventListener(evType, fn, useCapture);return true}else if (elm.attachEvent){var r = elm.attachEvent('on' + evType, fn);return r}else {elm['on' + evType] = fn}};contextMenuTab.prototype.addItem = function(id,name,control,type,handler)
...
{this.contextMenu.hideDiv.style.display = "inline";this.contextMenu.TabContainer.style.visibility = "visible";var self = this;this.contextMenu.tabPanel.firstChild.rows[0].cells[0].innerHTML = this.Description;this.contextMenu.tabPanel.style.visibility = "visible";var t = this.contextMenu.tabPanel.firstChild.rows[1].cells[0].firstChild;var c,c2,r = null

 if(this.showObjectInfo){if(this.object){if(this.object.constructor == GanttTask){this.insertData(t,"Id",this.object.TaskInfo.Id);this.insertData(t,"Name",this.object.TaskInfo.Name);this.insertData(t,"Duration",this.object.TaskInfo.Duration+" hrs");this.insertData(t,"Percent complete",this.object.TaskInfo.PercentCompleted+"%");this.insertData(t,"EST",this.object.TaskInfo.EST.getDate()+ "." + (this.object.TaskInfo.EST.getMonth() + 1) + "." + this.object.TaskInfo.EST.getFullYear());this.insertData(t,"Predecessor",this.object.TaskInfo.PredecessorTaskId)}else
 ...
 {r = t.insertRow(t.rows.length);c = r.insertCell(r.cells.length);c2 = r.insertCell(r.cells.length);c.innerHTML = this.arrItems[i].Name;c2.appendChild(this.arrItems[i].control)}};var b = document.createElement("input");b.type = "button";b.value = "Cancel";var f = function (e)
 {self.hide()};b.onclick = f;if (!btnCell){r = t.insertRow(t.rows.length);c = r.insertCell(r.cells.length);btnCell = r.insertCell(r.cells.length)}else {b.style.marginLeft = "10px"};btnCell.appendChild(b)};contextMenuTab.prototype.hide = function()
