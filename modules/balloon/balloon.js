...
*/
{
	...
	_draw: function() {
        ...
        var htmlCode = "<div id='upperLeftCorner_" + this.id + "' class='balloon_upperLeftCorner'></div>" +
                       ...
                       "<div id='messageBox_" + this.id + "' class='balloon_messageBox'>" +
	...
	show: function() {
		if(!this.visible && this.correctOptions) {
		    ...
		    document.observe('mouseup',this._checkOutside.bind(this));
			this._afterShow();
	...
	hide: function() {
		if(this.visible && this.correctOptions) {
		    ...
		    document.stopObserving('mouseup',this._checkOutside.bind(this));
		}
	...
	_resize: function() {
        if ((Object.isEmpty(this.dimensions[0])) && (Object.isEmpty(this.dimensions[1]))) {
			...
			messageBox.update(new Element("div", {
				id: "button_" + this.id,
				"class": "application_rounded_close"
			}));
            ...
            // Reset content value:
            Event.stopObserving($('button_' + this.id), 'click', this.hide.bindAsEventListener(this));
		    $('messageBox_' + this.id).update("<div id='button_" + this.id + "' class='application_rounded_close'></div>" + this.content);
		    Event.observe($('button_' + this.id), 'click', this.hide.bindAsEventListener(this));
		}
