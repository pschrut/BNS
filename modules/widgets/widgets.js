...
Object.extend(Widgets.Widget.prototype, {
    ...
    */
    ...
    initialize: function(widgetId, options) {
        ...
        */
        ...
        var divHeader = DIV({
            className: this.className + '_nw'
        }, [this._titleDiv, buttonContainer]);
        ...
        if (this.options.optionsButton) {
            ...
            this.optionsButton = new Element('div', {
                'class': 'application_rounded_options menus_widget_align_icon',
        ...
        document.observe('EWS:Widgets_contextMenuClick', function(event) {
            var args = getArgs(event);
            if (args != this._id) this.contextMenu.container.hide();
        } .bind(this));
    ...
    */
    insertContextMenuItem: function(name, className, callbackFunction, popup, dimensions) {
        ...
        if (popup) {
            this.menuOptionsPanels[this._panelNum].content = new Element('div', {
                'class': 'widget_optionsPanelPopup'
            });
            this.menuOptionsPanels[this._panelNum].element = this.createPopupPanel().insert(this.menuOptionsPanels[this._panelNum].content);
            this.menuOptionsPanels[this._panelNum].element.setStyle({
                height: dimensions.height,
                width: dimensions.width
            });
        }
        else {
            this.menuOptionsPanels[this._panelNum].element = new Element('div', {
                'class': 'widgets_optionsPanel',
                zIndex: 2
            });
            this.menuOptionsPanels[this._panelNum].content = this.menuOptionsPanels[this._panelNum].element;
        }
        this.menuOptionsPanels[this._panelNum].element.hide();
        this.menuOptionsPanels[this._panelNum].hide = true;
        this.menuOptionsPanels[this._panelNum].popup = popup;
        if (popup)
            $(document.body).insert(this.menuOptionsPanels[this._panelNum].element);
        else
            this.optionsPanelsContainer.insert(this.menuOptionsPanels[this._panelNum].element);
    ...
    /*
    * @method showOptionsPanel
    * @desc Determine which of the panels has to be shown and hide the rest of the panels
    * @param id {Integer} Id of the pannel
    */
    showOptionsPanel: function(id) {
        for (var i = 0; i < this._panelNum; i++) {
            if (i != id && !this.menuOptionsPanels[i].hide) {
                this.menuOptionsPanels[i].element.hide();
                this.menuOptionsPanels[i].hide = true;
            }
        }
        this.currentOptionsPanel = this.menuOptionsPanels[id];
        this.menuOptionsPanels[id].element.show();
        this.menuOptionsPanels[id].hide = false;
        if (this.menuOptionsPanels[id].popup) {
            Framework_stb.showSemitransparent();
            var width = this.menuOptionsPanels[id].element.getWidth();
            var height = this.menuOptionsPanels[id].element.getHeight();
            this.menuOptionsPanels[id].element.setStyle({
    ...
    */
    setTitle: function(title) {
        $(this._titleDiv).update('<abbr title="' + title + '"><span class="widgets_header_text application_text_bolder">' + title + '</span></abbr>');
        this.title = title;
        return this;
    },
    /*
    ...
    */
    ...
    setColor: function(className){        
        if(!Object.isEmpty(className)){
            this.divHeader.className = this.divHeader.className + ' barInColor_'+className;
            this._footerDiv.up().className = this._footerDiv.up().className + ' application_border_widget_bottom_'+className+ ' application_border_widget_'+className;
            this.divContent.className = this.divContent.className + ' application_border_widget_'+className;
        }else{
            this.divHeader.className = this.divHeader.className + ' application_header_bar';
            this._footerDiv.up().className = this._footerDiv.up().className + ' application_border_widget_default application_border_widget_bottom_default';
            this.divContent.className = this.divContent.className + ' application_border_widget_default';
        }
    },
    /*
    * @method setContent
